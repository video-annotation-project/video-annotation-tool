...
def fix_offset(annotation):
   ...
   cap.set(0, (annotation.timeinvideo-search_seconds_range)*1000) # tell video to start at 'start'-1 time
   ...
   for index, img in iter_from_middle(imgs):
      if index == len(imgs):
          continue
      (score, _) = compare_ssim(img, imgs[index], full=True, multichannel=True)
